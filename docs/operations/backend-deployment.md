...
백엔드는 Spring Boot 기반 API 서버이며, Docker 이미지로 패키징한 뒤 Backend EC2에서 Blue/Green 방식으로 배포한다. 데이터베이스는 AWS RDS MySQL을 사용하고, Redis는 별도 EC2 인스턴스에서 운영한다. 파일과 객체 저장소는 AWS S3를 사용한다.
...
| 외부 인프라 분리 | RDS, Redis EC2, S3는 API 서버 외부 인프라로 둔다. |
...
| 배포 전환 기준 — RDS, Redis, S3 연결을 포함한 컨테이너 readiness 검증 | `/api/actuator/health/readiness` |
...
`/api/health`는 애플리케이션 프로세스가 요청을 받을 수 있는지 확인하는 용도로만 사용한다. RDS, Redis, S3 연결 상태는 이 endpoint에서 확인하지 않는다. 외부 의존성 검증은 `/api/actuator/health/readiness`에서 수행한다.
...
| Redis EC2 생성 | Redis 설치, 보안 설정, persistence 설정은 별도 인프라 문서에서 관리한다. |
| S3 버킷 생성 | 버킷 생성, bucket policy, lifecycle 설정은 별도 인프라 문서에서 관리한다. |
| 도메인/DNS 설정 | API 도메인 연결은 인프라 설정 문서에서 다룬다. |

## 3. 인프라 구성

운영 인프라는 Backend EC2, AWS RDS MySQL, Redis EC2, AWS S3로 구성한다.
...
| Redis EC2 | 캐시, 토큰, 세션성 데이터, 임시 데이터 저장 |
...
| Redis EC2 | Backend EC2 보안 그룹에서 오는 `6379`만 허용 |
...
REDIS_HOST=<redis-private-ip-or-domain>
REDIS_PORT=6379
...
management:
  ...
  endpoint:
    health:
      ...
      group:
        readiness:
          include: readinessState,db,redis,s3
...
| indicator 이름 | `s3` (readiness group의 `include: readinessState,db,redis,s3`와 일치해야 한다) |
...
`application-prod.yml`은 환경 변수 placeholder만 포함하므로 Git repository에 커밋해도 시크릿이 직접 노출되지 않는다. 단, DB URL 패턴, Redis 포트, Actuator 경로 등 인프라 구조가 간접적으로 노출될 수 있으므로 repository는 private으로 운영하는 것을 권장한다. public repository를 사용하는 경우 이 파일이 노출하는 정보 범위를 사전에 검토한다.
...
| Redis 연결 | 인증, 세션, 토큰, 분산 락, 큐 등 핵심 기능에 사용하면 필수, Actuator `redis` health indicator로 검증 |
| S3 접근 | 필수, custom `HealthIndicator`를 readiness group에 포함해 검증 |

Redis가 단순 캐시이고 fallback이 가능하다면 readiness 필수 조건에서 제외할 수 있다. S3는 핵심 기능이므로 warning으로만 처리하지 않는다. 문서나 배포 스크립트에서 "S3 readiness 검증"이라고 표현할 경우에는 반드시 S3 custom health check가 구현되어 있어야 한다. S3 custom `HealthIndicator`의 구체적인 구현 명세(확인 방식, timeout, 반환 상태 등)는 4. 런타임 구성의 S3 custom HealthIndicator 구현 명세를 따른다. S3 upload/download smoke test는 readiness의 대체 수단이 아니라, Nginx 전환 후 이전 slot을 종료하기 전에 수행하는 추가 검증이다.
...
초기 몇 회의 실패는 애플리케이션 부팅, RDS connection pool 초기화, Redis 연결, S3 health check 지연으로 발생할 수 있으므로 즉시 실패로 보지 않는다. 다만 2분 안에 readiness가 성공하지 못하면 해당 컨테이너는 배포 불가 상태로 판단한다.
...
Redis 상태        -> 애플리케이션 health 또는 Redis exporter
...
새 컨테이너 실행 성공
readiness 성공 (RDS, Redis, S3 custom HealthIndicator 포함)
...
8.  RDS와 Redis는 새 컨테이너 readiness에서 검증한다.
