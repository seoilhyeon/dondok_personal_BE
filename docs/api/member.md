...
- `is_host_ever`, `hosted_crew_count`는 read-time 계산 projection이다. 방장으로 만든 크루 중 `CANCELLED`(해체)를 제외한 크루(`RECRUITING`/`ACTIVE`/`CLOSED`)를 카운트한다. 모집중인 크루의 방장도 운영 콘솔에 진입해야 하므로 뱃지 산정에 포함하며, 모집중에서 해체되어 `CANCELLED`로 남은 크루만 산정에서 제외한다.
