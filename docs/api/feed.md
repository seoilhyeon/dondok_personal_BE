...
{
  "available_crews": [
    { "crew_id": 42, "crew_name": "갓생 6시 기상", "status": "ACTIVE" },
    { "crew_id": 43, "crew_name": "독서 1챕터", "status": "ACTIVE" }
  ],
  "feed_items": [
    {
      "mission_log_id": 9001,
...
- `available_crews[]`는 필터 칩 구성을 위한 **호출자 참여 크루 목록**(`crew_id`, `crew_name`, `status`)이다. `status`는 크루 상태(`RECRUITING` / `ACTIVE` / `CLOSED` / `CANCELLED`)다. "전체 크루" 칩은 클라이언트가 구성한다.
