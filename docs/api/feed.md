...
- `feed_items[]`에는 `certification_status`가 `SUCCESS`, `PENDING_REVIEW`, `FAILED`인 로그를 모두 포함한다. `status` 쿼리 파라미터로 특정 상태만 필터링할 수 있다.
- 미제출일(`NOT_SUBMITTED`)은 `feed_items[]`에 포함하지 않는다.
- `reject_reason_code`는 호스트 검수 거절 사유이며, 해당 없으면 `null`이다.
...
- `(mission_log_id, member_id, reaction_type)` unique constraint 기반의 DB 레벨 멱등성을 보장해야 한다. MySQL 8.0 문법을 따른다.
