...
--  → 커밋 후 즉시 시도(@TransactionalEventListener AFTER_COMMIT)가 유실/실패해도 배치가 주워 EXIF 제거를 보장한다.
-- status/retry_count/last_error로 실패를 가시화하고, attempt_version으로 stale 결과 기록을 방어한다.
CREATE TABLE image_reencode_task (
    ...
    attempt_version    BIGINT          NOT NULL DEFAULT 0,                  -- claim fencing token (claim마다 증가, stale 결과 방어)
