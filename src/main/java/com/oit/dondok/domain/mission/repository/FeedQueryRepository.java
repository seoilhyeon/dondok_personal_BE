...
public class FeedQueryRepository {
  ...
  public List<AvailableCrewResponse> findParticipatingCrews(UUID
      memberUuid) {
    return queryFactory
            .select(Projections.constructor(AvailableCrewResponse.class,
                    crew.id, crew.title))
  ...
  private BooleanExpression goeOrNull(LocalDateTime fromInclusive) {
    return fromInclusive == null ? null :
            missionLog.serverTime.goe(fromInclusive);
  }

  private BooleanExpression ltOrNull(LocalDateTime toExclusive) {
    return toExclusive == null ? null :
            missionLog.serverTime.lt(toExclusive);
  }
